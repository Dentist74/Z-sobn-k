...
import { acceptInvite, type FormState } from "@/app/actions/onboarding";
...
export function AcceptInviteForm({
  ...
  const [state, action, pending] = useActionState<FormState, FormData>(acceptInvite, undefined);
  ...
  return (
    <div className="w-full rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
      <div className="mb-5 flex flex-col items-center text-center">
        ...
        <h1 className="mt-3 text-lg font-semibold text-slate-900">Připojení k Zásobníku</h1>
        <p className="mt-1 text-sm text-slate-500">
          Role: <span className="font-medium text-[#103D63]">{roleLabel}</span>
        </p>
      </div>
      ...
      <form action={action} className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <input id="name" name="name" required autoFocus className={inputClass} placeholder="Jana Nováková" />
        </div>
        <div className="space-y-1.5">
          <label htmlFor="email" className="text-sm font-medium text-slate-700">E-mail</label>
          <input id="email" name="email" type="email" required className={inputClass}
            defaultValue={email ?? ""} readOnly={!!email} placeholder="jmeno@klinika.cz" />
        </div>
        <div className="space-y-1.5">
        ...
        <button type="submit" disabled={pending}
          ...
          {pending ? "Zakládám účet…" : "Vytvořit účet a přihlásit"}
        </button>
      </form>
    </div>
  );
